...
mur_multivariate <- function(X, Y, extract){
  ...
  for(y in 1:nPoints){
    ...
    results <- manova(as.formula(model_formula), data = dat)
    ss <- summary(model)
    # Calculate effect sizes
    effect_size <- eta_squared(results)
    
    for(iEx in 1:length(extract)){
      
      tMUR[y,1+(iEx-1)*3] <- effect_size$Eta2_partial[iEx]  # Eta squared
      tMUR[y,2+(iEx-1)*3] <- ss[iEx, 3]  # approx F
      tMUR[y,3+(iEx-1)*3] <- ss[iEx, 6]  # p-value
    }
